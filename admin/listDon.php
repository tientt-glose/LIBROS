  ...
  <tbody>
    ...
    <tr>
      <td><table width="1048" border="1">
        <tbody>
          <tr>
            <td colspan="10" style="text-align: center; font-weight: 600; font-size: 30px;">DANH SÁCH BOOK</td>
          </tr>
          ...
          <?php
			...
			$sach = DanhSachBook_page($from,$sosach1trang);
			//search ve lenh
			while($row_sach=mysqli_fetch_array($sach)) {
				ob_start();
			?>
          <tr>
            <td rowspan="2" style="text-align: center">{IDSach}</td>
            <td rowspan="2" style="text-align: center"><a href="suaSach.php?trang=<?php echo $trang?>&id={IDSach}">{Tensach}</a><br>
              <img style='max-width: 100%; height: auto;' src="<?php $img= explode(".", $row_sach["Hinhanh"]); 
			   ...
			   alt=""  id="imgS"/></td>
            <td rowspan="2" style="text-align: center">{Tacgia}  </td>
            <td rowspan="2" style="text-align: center">{Theloai}</td>
            <td rowspan="2" style="text-align: center">{NXB}</td>
            <td rowspan="2" style="text-align: center">{Sotrang}</td>
            <td rowspan="2" style="text-align: center">{NamXB}</td>
            <td rowspan="2" style="text-align: center">{Giasach}</td>
            <td style="text-align: center">{AnHien}</td>
            <td rowspan="2"><a href="suaSach.php?trang=<?php echo $trang?>&id={IDSach}">Sửa</a> - <a onclick="return confirm('Bạn có chắc là muốn xóa không?')" href="xoaSach.php?trang=<?php echo $trang?>&id={IDSach}">Xóa</a></td>
          </tr>
          <tr>
            <td style="text-align: center">{SLTon}</td>
          </tr>
          <?php 
				// search ve lenh
			$s = ob_get_clean();
			...
			$s = str_replace("{urlHINH}",$row_sach["Hinhanh"],$s);
				...
				if ($row_sach["SLTon"]!=0) $s = str_replace("{SLTon}","Còn",$s);
				else $s = str_replace("{SLTon}","Hết",$s);
